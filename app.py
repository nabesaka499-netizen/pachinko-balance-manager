...
if len(df) > 0:
    ...
    total_time = float(df['遊技時間'].sum())
    total_balance = float(df['収支'].sum())
    avg_hourly = int(total_balance / total_time) if total_time > 0 else 0
    
    with col1:
        st.metric("💰 総収支", f"{int(total_balance):,}円")
    with col2:
        st.metric("💳 総入金額", f"{int(df['入金額'].sum()):,}円")
    with col3:
        st.metric("⏱️ 総遊技時間", f"{total_time:.1f}時間")
    with col4:
    ...
    # 修正：時給計算でゼロ除算を回避
    monthly_summary['平均時給'] = monthly_summary.apply(
        lambda row: int(row['収支'] / row['遊技時間']) if row['遊技時間'] > 0 else 0,
        axis=1
    )
